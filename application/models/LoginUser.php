...
class LoginUser extends CI_model
{
	...
	* @getConnection : verification d'authentification
    ...
    public function getConnection($login,$pwd)
	{
    	...
    	if ($rslt == 1) 
    	{
    	    $verify = password_verify($pwd, $stmt[0]->password);
            if ($verify){
                ...
                $user = new Utilisateur($nom_user, $prenom_user, $email_user, $phone_user, $id_user, $privilege);
...
//debug($shop);
                ...
                else
                {
                    ...
                    //Stockage dans les variables
                    $id_client = $stmt[0]->id_customer;
                    $created_at = $stmt[0]->customer_created_at;
                    //___Instantiation de l'objet Vendeur
            ...
            else
            {
                ...
                if($_SESSION['site_lang'] == "french"){
                    $_SESSION['msg'] = 'Email ou mot de passe incorrects !'.$pwd;
                }
                else{
