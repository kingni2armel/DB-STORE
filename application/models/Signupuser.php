...
//___Sauvegarde les informations de l'utilisateur dans la base de donnees
    ...
    public function updateCustomerInformations($id, $nom, $prenom, $email, $phone, $password)
    {
...
//___si les donnees sur l'utilisateur sont differente de celle enregistré
        if ($nom != $nom_user || $prenom != $prenom_user || $email != $email_user || $phone != $phone_user || $encrypt_pwd != $password_user)
        {
            $data = array(
                'nom_user' => $nom,
                'prenom_user' => $prenom,
...
//___creation d'un message
            if ($_SESSION['site_lang'] == 'french') {
                $_SESSION['msg'] = 'Vos informations ont ete mis a jour !'.password_hash($password,PASSWORD_DEFAULT);
            }
            else{
                $_SESSION['msg'] = 'Your informations has been update !'.$password;
            }
//___redirection a vers la page de connection
        ...
        {
            if ($_SESSION['site_lang'] == 'french') {
                $_SESSION['msg'] = "Vous n'avez modifié aucun champ !".$password;
            }
            else{
    ...
    public function updateSellerInformations($nom, $prenom, $email, $phone, $password, $shopname, $shoptown, $shopstreet, $description)
    {
...
//___Si les information des differents champs sont egaux a ceux de la vas de donnees 
        if ($nom == $nom_user && $prenom == $prenom_user && $email == $email_user && $phone == $phone_user && password_hash($password) == $password_user && $shopname ==$name_shop && $shopstreet ==$street_shop  && $shoptown == $city_shop && $description == $description_shop)
        {
            if ($_SESSION['site_lang'] == 'french') {
