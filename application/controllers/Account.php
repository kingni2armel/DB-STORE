...
//___Chargement des variables 
        if($this->form_validation->run()==true)
        {
            ...
            if ($_SESSION['privilege'] == 'customer'){
                $this->signupuser->updateCustomerInformations($_SESSION['id_user'],$nom, $prenom, $email, $phone, $password );
            }
            else
            {
               $deb = $this->signupuser->updateSellerInformations($nom, $prenom, $email, $phone, $password, $shopname, $shoptown, $shopstreet, $shopdescription);
